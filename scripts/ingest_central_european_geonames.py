...
Downloads and processes GeoNames dump files for Germany (DE), Austria (AT),
Switzerland (CH), Poland (PL), and Czech Republic (CZ). Filters to features
of toponymic interest and writes JSONL output per country.

Target: ~20,000–30,000 records total across 5 countries.

Usage:
    uv run python scripts/ingest_central_european_geonames.py
"""
...
# Feature class/code filters for smaller countries (AT, CH, PL, CZ)
STANDARD_P_CODES = None  # All P.* subtypes
STANDARD_T_CODES = None  # All T.* subtypes
STANDARD_H_CODES = {"STM", "LK", "RSVR", "RVR"}
STANDARD_S_CODES = {"FRM", "CSTL", "CH", "MNST", "RSTN", "AIRP", "HSE", "MN", "BLDG", "ANS"}
STANDARD_L_CODES = {"RGN", "AREA", "PRK", "RESN", "CONT", "LCTY"}

# Germany-specific: more selective H codes to manage volume
DE_H_CODES = {"STM", "LK", "RSVR", "RVR", "STRT", "BAY", "COVE", "FJD"}

# Per-country caps to keep total around 20,000-25,000 records
COUNTRY_CAPS = {
    "DE": {"p_cap": 3000, "t_cap": 2000, "other_cap": 500},
    "AT": {"p_cap": 2000, "t_cap": 2000, "other_cap": 500},
    "CH": {"p_cap": 2000, "t_cap": 2000, "other_cap": 500},
    "PL": {"p_cap": 3000, "t_cap": 1500, "other_cap": 500},
    "CZ": {"p_cap": 2000, "t_cap": 1500, "other_cap": 500},
}
...
def should_include_non_p(feature_class: str, feature_code: str) -> bool:
    ...
    if feature_class == "H":
        return feature_code in DE_H_CODES
    if feature_class == "S":
