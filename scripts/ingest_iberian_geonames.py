#!/usr/bin/env python3
"""Ingest GeoNames data for the Iberian Peninsula.

Downloads and processes GeoNames dump files for Spain (ES), Portugal (PT),
Andorra (AD), and Gibraltar (GI). Filters to features of toponymic interest
with emphasis on Arabic substrate layer (al-/Guadal-/Beni- prefixes).

Target: ~10,000–15,000 records total across 4 countries.

Usage:
    uv run python scripts/ingest_iberian_geonames.py
"""
...
# Per-country caps
COUNTRY_CAPS = {
    "ES": {"p_cap": 5000, "t_cap": 2500, "other_cap": 1000},
    "PT": {"p_cap": 2000, "t_cap": 1000, "other_cap": 500},
    "AD": {"p_cap": 200, "t_cap": 100, "other_cap": 50},
    "GI": {"p_cap": 50, "t_cap": 20, "other_cap": 20},
}
